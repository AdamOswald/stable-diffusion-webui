...
def smergegen(
    ...
    result, currentmodel, modelid, theta_0 = smerge(
        weights_a,
        weights_b,
        model_a,
        model_b,
...
def smerge(
    ...
    for key in tqdm(theta_0.keys(), desc="Stage 1/2") if not False else theta_0.keys():
        if "model" in key and key in theta_1:
            ...
            if modes[1] in mode:  # Add
                caster(
                    f"model A[{key}] +  {current_alpha} + * (model B - model C)[{key}]",
                    hear,
                )
                theta_0[key] = theta_0[key] + current_alpha * theta_1[key]
            elif modes[2] in mode:  # Triple
                caster(
                    f"model A[{key}] +  {1-current_alpha-current_beta} +  model B[{key}]*{current_alpha} + model C[{key}]*{current_beta}",
                    hear,
                )
                theta_0[key] = (
...
def makemodelname(
    ...
    if useblocks:
        ...
        if modes[1] in mode:  # add
            currentmodel = (
                f"{model_a} + ({model_b} -  {model_c}) x {str(round(alpha,3))}"
            )
        elif modes[2] in mode:  # triple
