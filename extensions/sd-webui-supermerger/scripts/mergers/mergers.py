...
mergedmodel = []
typesg = ["none", "alpha", "beta (if Triple or Twice is not selected,Twice automatically enable)", "alpha and beta", "seed", "mbw alpha", "mbw beta", "mbw alpha and beta",
          "model_A", "model_B", "model_C", "pinpoint blocks (alpha or beta must be selected for another axis)", "elemental", "pinpoint element", "effective elemental checker"]
types = ["none", "alpha", "beta", "alpha and beta", "seed", "mbw alpha ", "mbw beta", "mbw alpha and beta",
         "model_A", "model_B", "model_C", "pinpoint blocks", "elemental", "pinpoint element", "effective"]
...
def smergegen(weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, mode, useblocks, custom_name, save_sets, id_sets, wpresets, deep, esettings,
    ...
    result, currentmodel, modelid, theta_0 = smerge(weights_a, weights_b, model_a, model_b, model_c, base_alpha,
                                                    base_beta, mode, useblocks, custom_name, save_sets, id_sets, wpresets, deep, deepprint=deepprint)
    ...
    result = savemodel(theta_0, currentmodel, custom_name, save_sets,
                       model_a) if save else "Merged model loaded:"+currentmodel

    gc.collect()

    if imggen:
        images = simggen(prompt, nprompt, steps, sampler, cfg,
                         seed, w, h, currentmodel, id_sets, modelid)
...
def smerge(weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, mode, useblocks, custom_name, save_sets, id_sets, wpresets, deep, deepprint=False):
    ...
    if type(base_alpha) == str:
        base_alpha = float(base_alpha)
    if type(base_beta) == str:
    ...
    mergedmodel = [weights_a, weights_b,
                   hashfromname(model_a), hashfromname(
                       model_b), hashfromname(model_c),
        ...
        if "model" in key and key in theta_1:
            ...
            if len(deep) > 0:
                ...
                for d in deep:
                    if d.count(":") != 2:
                        continue
                    dbs, dws, dr = d.split(":")[0], d.split(":")[
                        1], d.split(":")[2]
                    dbs, dws = dbs.split(" "), dws.split(" ")
                    dbn, dbs = (True, dbs[1:]) if dbs[0] == "NOT" else (
                        False, dbs)
                    dwn, dws = (True, dws[1:]) if dws[0] == "NOT" else (
                        False, dws)
                    flag = dbn
                    for db in dbs:
                        if db in skey:
            ...
            if modes[1] in mode:  # Add
                caster(
                    f"model A[{key}] +  {current_alpha} + * (model B - model C)[{key}]", hear)
                theta_0[key] = theta_0[key] + current_alpha * theta_1[key]
            elif modes[2] in mode:  # Triple
                caster(
                    f"model A[{key}] +  {1-current_alpha-current_beta} +  model B[{key}]*{current_alpha} + model C[{key}]*{current_beta}", hear)
                theta_0[key] = (1 - current_alpha-current_beta) * theta_0[key] + \
                    current_alpha * theta_1[key]+current_beta * theta_2[key]
            elif modes[3] in mode:  # Twice
                caster(
                    f"model A[{key}] +  {1-current_alpha} + * model B[{key}]*{alpha}", hear)
                caster(
                    f"model A+B[{key}] +  {1-current_beta} + * model C[{key}]*{beta}", hear)
    ...
    currentmodel = makemodelname(
        weights_a, weights_b, model_a, model_b, model_c, base_alpha, base_beta, useblocks, mode)
...
def simggen(prompt, nprompt, steps, sampler, cfg, seed, w, h, mergeinfo="", id_sets=[], modelid="no id"):
    ...
    if "image" in id_sets:
        processed.images[0] = draw_origin(
            processed.images[0], str(modelid), w, h, w)
    ...
    images.save_image(image, opts.outdir_txt2img_samples, "", p.seed,
                      p.prompt, shared.opts.samples_format, p=p, info=infotext)
    shared.state.end()
