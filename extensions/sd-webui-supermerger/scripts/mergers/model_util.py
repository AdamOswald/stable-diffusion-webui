...
def convert_ldm_unet_checkpoint(v2, checkpoint, config):
    ...
    num_input_blocks = len({".".join(layer.split(
        ".")[:2]) for layer in unet_state_dict if "input_blocks" in layer})
    input_blocks = {
        layer_id: [
    ...
    num_middle_blocks = len({".".join(layer.split(
        ".")[:2]) for layer in unet_state_dict if "middle_block" in layer})
    middle_blocks = {
        layer_id: [
    ...
    num_output_blocks = len({".".join(layer.split(
        ".")[:2]) for layer in unet_state_dict if "output_blocks" in layer})
    output_blocks = {
        layer_id: [
    ...
    for i in range(1, num_input_blocks):
        ...
        attentions = [key for key in input_blocks[i]
                      if f"input_blocks.{i}.1" in key]
    ...
    assign_to_checkpoint(resnet_0_paths, new_checkpoint,
                         unet_state_dict, config=config)

    resnet_1_paths = renew_resnet_paths(resnet_1)
    assign_to_checkpoint(resnet_1_paths, new_checkpoint,
                         unet_state_dict, config=config)

    attentions_paths = renew_attention_paths(attentions)
    meta_path = {"old": "middle_block.1", "new": "mid_block.attentions.0"}
    assign_to_checkpoint(
        attentions_paths, new_checkpoint, unet_state_dict, additional_replacements=[
            meta_path], config=config
    )
    ...
    for i in range(num_output_blocks):
        ...
        output_block_layers = [shave_segments(
            name, 2) for name in output_blocks[i]]
        output_block_list = {}

        for layer in output_block_layers:
            layer_id, layer_name = layer.split(
                ".")[0], shave_segments(layer, 1)
        ...
        if len(output_block_list) > 1:
            resnets = [key for key in output_blocks[i]
            ...
            meta_path = {"old": f"output_blocks.{i}.0",
...
def create_unet_diffusers_config(v2):
    ...
    block_out_channels = [UNET_PARAMS_MODEL_CHANNELS *
                          mult for mult in UNET_PARAMS_CHANNEL_MULT]
...
def convert_ldm_clip_checkpoint_v2(checkpoint, max_length):
    ...
    def convert_key(key):
        ...
        if "resblocks" in key:
            ...
            key = key.replace(".positional_embedding",
            ...
            key = key.replace(".token_embedding.weight",
    ...
    for key in keys:
        ...
        if '.resblocks' in key and '.attn.in_proj_' in key:
            ...
            key_pfx = key.replace(
                "cond_stage_model.model.transformer.resblocks.", "text_model.encoder.layers.")
...
def savemodel(state_dict, currentmodel, fname, savesets, model_a):
    ...
    if not fname or fname == "":
        fname = currentmodel.replace(" ", "").replace(
            ",", "_").replace("(", "_").replace(")", "_")+pre+ext
        if fname[0] == "_":
            fname = fname[1:]
    else:
    ...
    if ext == ".safetensors":
        safetensors.torch.save_file(
            state_dict, fname, metadata={"format": "pt"})
    else:
...
def setup_for_low_vram_s(sd_model, use_medvram):
    ...
    stored = sd_model.first_stage_model, getattr(
