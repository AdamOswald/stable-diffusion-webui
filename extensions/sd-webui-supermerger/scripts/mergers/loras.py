...
def merge_to_sd_model(theta_0, merge_dtype, lorans, lorars, model_a):
    ...
    return save_stable_diffusion_checkpoint(False, "", text_encoder, unet,
                                            model_a, 0, 0, merge_dtype, vae)
...
def create_unet_diffusers_config(v2):
    ...
    for i in range(len(block_out_channels)):
        block_type = ("CrossAttnDownBlock2D" if resolution
                      in UNET_PARAMS_ATTENTION_RESOLUTIONS else "DownBlock2D")
    ...
    up_block_types = []
    for i in range(len(block_out_channels)):
        block_type = ("CrossAttnUpBlock2D" if resolution
                      in UNET_PARAMS_ATTENTION_RESOLUTIONS else "UpBlock2D")
...
def convert_text_encoder_state_dict_to_sd_v2(checkpoint,
    ...
    def convert_key(key):
        ...
        if "layers" in key:
            ...
            key = key.replace("embeddings.token_embedding.weight",
    ...
    for key in keys:
        if "layers" in key and "q_proj" in key:
            ...
            new_key = key.replace("text_model.encoder.layers.",
    ...
    if make_dummy_weights:
        ...
        new_sd["text_projection"] = torch.ones((1024, 1024),
