...
def sgenxyplot(
    ...
    if xmen == "":
        return "ERROR: parameter X is empty", *None5
    if ymen == "" and not ytype == "none":
        return "ERROR: parameter Y is empty", *None5
    if model_a == [] and not ("model_A" in xtype or "model_A" in ytype):
    ...
    def dicedealer(zs):
        for i, z in enumerate(zs):
            if z == "-1":
                zs[i] = str(random.randrange(4294967294))
    ...
    def xydealer(z, zt, azt):
        ...
        if "and" in zt:
            ...
            return
        if "alpha" in zt and not "pinpoint element" in azt:
            alpha = z
        if "beta" in zt:
            beta = z
        if "seed" in zt:
    ...
    if "mbw alpha and beta" in xtype:
        xs = [f"alpha:({x[0]}),beta({x[1]})" for x in xs]
...
def makegridmodelname(
    ...
    if "mbw" in xtype:
        if "alpha" in xtype:
            wa = "X"
        if usebeta or " beta" in xtype:
            wb = "X"

    if "mbw" in ytype:
        if "alpha" in ytype:
            wa = "Y"
        if usebeta or " beta" in ytype:
            wb = "Y"
    ...
    if "mbw" in xtype:
        if "alpha" in xtype:
            alpha = "X"
        if "beta" in xtype or usebeta:
            beta = "X"

    if "mbw" in ytype:
        if "alpha" in ytype:
            alpha = "Y"
        if "beta" in ytype or usebeta:
            beta = "Y"
